...
//
// Português: Defina este sinalizador para um valor maior ou menor que o padrão de 1024 para aumentar ou reduzir o
// peso do container e dar a ele acesso a uma proporção maior ou menor dos ciclos de CPU da máquina host.
// Isso só é aplicado quando os ciclos da CPU são restritos. Quando muitos ciclos de CPU estão disponíveis, todos
// os container usam a quantidade de CPU de que precisam. Dessa forma, este é um limite flexível. --cpu-shares não
// impede que os containers sejam agendados no modo swarm.
