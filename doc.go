...
//
//   Nota: Para conseguir acessar repositórios privados de dentro do container, construa a imagem em duas ou mais etapas
//   e na primeira etapa, copie os arquivos id_rsa e known_hosts para a pasta /root/.ssh e o arquivo .gitconfig para a
//   pasta /root/.
//
//   Uma forma de fazer isto de forma automática é usar o exemplo de Dockerfile abaixo, onde os argumentos
//   SSH_ID_RSA_FILE contém o arquivo .ssh/id_rsa, KNOWN_HOSTS_FILE contém o arquivo .ssh/known_hosts e GITCONFIG_FILE
//   contém o arquivo .gitconfig.
//
//   Caso a chave .ssh/id_rsa seja protegida com senha, use a função SetGitSshPassword() para definir a senha da mesma.
//
//   Caso você queira copiar os arquivos para dentro da imagem de forma automática, use SetPrivateRepositoryAutoConfig()
//   e a função vai copiar os arquivos ~/.ssh/id_rsa, ~/.ssh/known_hosts e ~/.gitconfig para as viáveis de argumentos
//   sitada anteriormente.
//
//   Caso queira definir os arquivos de forma manual, use SetGitConfigFile(), SetSshKnownHostsFile() e SetSshIdRsaFile()
//   para definir os arquivos de forma manual.
//
